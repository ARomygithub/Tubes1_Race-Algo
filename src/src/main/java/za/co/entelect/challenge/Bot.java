...
//        List<Object> blocks = getBlocksInFront(myCar.position.lane, myCar.position.block);
       if (gameState.player.damage >= 5) {
           bestCom = "FIX";
       }
       if (gameState.player.damage != 5 && gameState.player.speed <=5){
           bestCom = "ACCELERATE";
       }
...
//        System.out.printf("%d truck[0][0] %d truck[0][1] %d truck[1][0] %d truck[1][1]%n", truck[0][0], truck[0][1], truck[1][0], truck[1][1]);
    ...
    private void getPowerups (Result cur){
        ...
        xi = min(myX+resi.speed,end); yi = myY;

        if(xi== myX +resi.speed && gameState.lanes.get(yi-1)[xi].terrain == Terrain.EMP){
            bestCom = "TURN_LEFT";
        }
        if(xi== myX +resi.speed && gameState.lanes.get(yi-1)[xi].terrain == Terrain.OIL_POWER){
            bestCom = "TURN_LEFT";
        }

        if(xi== myX +resi.speed && gameState.lanes.get(yi-1)[xi].terrain == Terrain.TWEET){
            bestCom = "TURN_LEFT";
        }
        ...
        if(xi!= myX && xi== myX+resi.speed) {
            Terrain curTerrain = gameState.lanes.get(yi)[xi].terrain;
            if(curTerrain==Terrain.OIL_POWER) {
                ++resi.ctOil;
